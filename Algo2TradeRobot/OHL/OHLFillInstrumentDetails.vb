...
#End Region
    ...
    Private ReadOnly _parentStrategy As OHLStrategy
    ...
    Public Sub New(ByVal canceller As CancellationTokenSource, ByVal parentStrategy As OHLStrategy)
        _cts = canceller
                                                                                                   ...
                                                                                                   If y.RawExchange.ToUpper = "NFO" AndAlso (bannedStock Is Nothing OrElse
                                                                                                       ...
                                                                                                       If futureHistoricalCandlesJSONDict IsNot Nothing AndAlso futureHistoricalCandlesJSONDict.Count > 0 Then
                                                                                                           ...
                                                                                                           If futureEODPayload IsNot Nothing AndAlso futureEODPayload.Count > 0 Then
                                                                                                               ...
                                                                                                               If lastDayPayload.ClosePrice.Value >= 100 AndAlso lastDayPayload.ClosePrice.Value <= 3000 Then
                                                                                                                               ...
                                                                                                                               If atrPercentage >= 3 Then
                                                                                                                                   ...
                                                                                                                                   If volumePayload IsNot Nothing AndAlso volumePayload.Count > 0 Then
                                                                                                                                       ...
                                                                                                                                       Dim avgVolume As Decimal = volumePayload.Average(Function(x)
                                                                                                                                                                                        ...
                                                                                                                                                                                        End Function)
                                                                                                                                       _cts.Token.ThrowIfCancellationRequested()
                                                                                                                                       If avgVolume >= (0 / 100) * lastDayClosePrice Then
                                                                                                                                           If highATRStocks Is Nothing Then highATRStocks = New Concurrent.ConcurrentDictionary(Of String, Decimal())
                                                                                                                                           highATRStocks.TryAdd(instrumentData.Value, {atrPercentage, lastDayClosePrice})
                                                                                                                                       End If
                                                                                                                                   End If
                                                                                                                               End If
                                                                                                                           End If
                    ...
                    If capableStocks IsNot Nothing AndAlso capableStocks.Count > 0 Then
                        ...
                        If stocksLessThanMaxBlankCandlePercentage IsNot Nothing AndAlso stocksLessThanMaxBlankCandlePercentage.Count > 0 Then
                            ...
                            For Each stockData In stocksLessThanMaxBlankCandlePercentage.OrderByDescending(Function(x)
                                ...
                                stockCounter += 1
                                If stockCounter = 20 Then Exit For
                            Next
                        End If
                        _cts.Token.ThrowIfCancellationRequested()
                        If todayStockList IsNot Nothing AndAlso todayStockList.Count > 0 Then
                            ...
                            If CType(_parentStrategy.UserSettings, OHLUserInputs).InstrumentDetailsFilePath IsNot Nothing AndAlso
                                ...
                                File.Delete(CType(_parentStrategy.UserSettings, OHLUserInputs).InstrumentDetailsFilePath)
                                Using csv As New CSVHelper(CType(_parentStrategy.UserSettings, OHLUserInputs).InstrumentDetailsFilePath, ",", _cts)
                                    _cts.Token.ThrowIfCancellationRequested()
                                    ...
                                    For Each stock In todayStockList
                                        ...
                                        Dim instrument As IInstrument = allInstruments.ToList.Find(Function(x)
                                                                                                   ...
                                                                                                   End Function)
                                        row("Instrument Name") = instrument.TradingSymbol
                                        allStockData.Rows.Add(row)
                                    Next

                                    csv.GetCSVFromDataTable(allStockData)
                                End Using
                                If CType(_parentStrategy.UserSettings, OHLUserInputs).InstrumentsData IsNot Nothing Then
                                    CType(_parentStrategy.UserSettings, OHLUserInputs).InstrumentsData.Clear()
                                    CType(_parentStrategy.UserSettings, OHLUserInputs).InstrumentsData = Nothing
                                    CType(_parentStrategy.UserSettings, OHLUserInputs).FillInstrumentDetails(CType(_parentStrategy.UserSettings, OHLUserInputs).InstrumentDetailsFilePath, _cts)
                                End If
                            End If
                        End If
                    End If
