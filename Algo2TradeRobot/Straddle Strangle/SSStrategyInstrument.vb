...
Public Class SSStrategyInstrument
...
#End Region
    ...
    Private ReadOnly _SSFileName As String = Nothing

    Public StopInstrument As Boolean
    Public StopInstrumentReason As String = ""

    Public MyParentInstrumentDetails As SSUserInputs.InstrumentDetails
    Public TakeTrade As Boolean = False
    Public InstrumentOption As OptionType = OptionType.None
    ...
    Public Sub New(ByVal associatedInstrument As IInstrument,
        ...
        If Me.ParentStrategy.IsStrategyCandleStickBased Then
            Dim userInputs As SSUserInputs = Me.ParentStrategy.UserSettings
            Dim instrumentData As SSUserInputs.InstrumentDetails = Nothing
        ...
        _SSFileName = Path.Combine(My.Application.Info.DirectoryPath, String.Format("{0} {1}.StraddleStrangle.a2t", Me.TradableInstrument.TradingSymbol, Now.ToString("yy_MM_dd")))
        _tickerStatusFilename = Path.Combine(My.Application.Info.DirectoryPath, "TickerStatus.StraddleStrangle.a2t")
        ...
        Try
            ...
            If Me.TradableInstrument.InstrumentType <> IInstrument.TypeOfInstrument.Options Then
                ...
                If _myOptionStrategyInstruments IsNot Nothing AndAlso _myOptionStrategyInstruments.Count > 0 Then
                    Dim userSettings As SSUserInputs = Me.ParentStrategy.UserSettings
                    Dim instrumentDetails As SSUserInputs.InstrumentDetails = userSettings.InstrumentsData(Me.TradableInstrument.TradingSymbol.ToUpper)
                    While True
                        ...
                        If Me.TradableInstrument.LastTick IsNot Nothing AndAlso Me.TradableInstrument.LastTick.Timestamp.Value.Date = Now.Date AndAlso
                            Now >= instrumentDetails.EntryTime AndAlso Me.TradableInstrument.LastTick.Open > 0 Then
                            Dim currentTick As ITick = Me.TradableInstrument.LastTick
                            Dim ltp As Decimal = Decimal.MinValue
                            If Not File.Exists(_SSFileName) Then
                                ltp = currentTick.LastPrice

                                Utilities.Strings.SerializeFromCollection(Of Decimal)(_SSFileName, ltp)

                                OnHeartbeat(String.Format("{0}: LTP={1}, Time={2}. Now it will eliminate other option instruments.",
                                                          Me.TradableInstrument.TradingSymbol, ltp, currentTick.Timestamp.Value.ToString("dd-MM-yyyy HH:mm:ss")))
                            Else
                                ltp = Utilities.Strings.DeserializeToCollection(Of Decimal)(_SSFileName)
                            End If
                            ...
                            Dim atmStrike As Decimal = GetATMStrike(ltp, allStrikes)
                            Dim peStraddle As SSStrategyInstrument = _myOptionStrategyInstruments.Where(Function(x)
                                                                                                            Return x.TradableInstrument.Strike = atmStrike AndAlso
                                                                                                            x.TradableInstrument.RawInstrumentType = "PE"
                                                                                                        End Function).FirstOrDefault

                            Dim ceStraddle As SSStrategyInstrument = _myOptionStrategyInstruments.Where(Function(x)
                                                                                                            Return x.TradableInstrument.Strike = atmStrike AndAlso
                                                                                                            x.TradableInstrument.RawInstrumentType = "CE"
                                                                                                        End Function).FirstOrDefault

                            Dim peStrangleStrike As Decimal = GetATMStrike(atmStrike - instrumentDetails.StaddleToStrangleDistance, allStrikes)
                            Dim ceStrangleStrike As Decimal = GetATMStrike(atmStrike + instrumentDetails.StaddleToStrangleDistance, allStrikes)
                            Dim peStrangle As SSStrategyInstrument = _myOptionStrategyInstruments.Where(Function(x)
                                                                                                            Return x.TradableInstrument.Strike = peStrangleStrike AndAlso
                                                                                                            x.TradableInstrument.RawInstrumentType = "PE"
                                                                                                        End Function).FirstOrDefault

                            Dim ceStrangle As SSStrategyInstrument = _myOptionStrategyInstruments.Where(Function(x)
                                                                                                            Return x.TradableInstrument.Strike = ceStrangleStrike AndAlso
                                                                                                            x.TradableInstrument.RawInstrumentType = "CE"
                                                                                                        End Function).FirstOrDefault


                            For Each runningStrategyInstrument In _myOptionStrategyInstruments
                                If runningStrategyInstrument.TradableInstrument.TradingSymbol.ToUpper = peStraddle.TradableInstrument.TradingSymbol.ToUpper OrElse
                                   runningStrategyInstrument.TradableInstrument.TradingSymbol.ToUpper = ceStraddle.TradableInstrument.TradingSymbol.ToUpper OrElse
                                   runningStrategyInstrument.TradableInstrument.TradingSymbol.ToUpper = peStrangle.TradableInstrument.TradingSymbol.ToUpper OrElse
                                   runningStrategyInstrument.TradableInstrument.TradingSymbol.ToUpper = ceStrangle.TradableInstrument.TradingSymbol.ToUpper Then
                                    runningStrategyInstrument.TradableInstrument.FetchHistorical = True
                                Else
                                    CType(runningStrategyInstrument, SSStrategyInstrument).StopInstrumentReason = "+++ Not an ATM instrument"
                                    CType(runningStrategyInstrument, SSStrategyInstrument).StopInstrument = True
                                End If
                            Next

                            OnHeartbeat(String.Format("Selected Straddle PE={0}, Straddle CE={1}, Strangle PE={2}, Strangle CE={3}",
                                                       peStraddle.TradableInstrument.TradingSymbol,
                                                       ceStraddle.TradableInstrument.TradingSymbol,
    ...
    Protected Overrides Async Function IsTriggerReceivedForPlaceOrderAsync(forcePrint As Boolean) As Task(Of List(Of Tuple(Of ExecuteCommandAction, PlaceOrderParameters, String)))
        ...
        If currentTime >= userSettings.TradeStartTime AndAlso runningCandlePayload IsNot Nothing AndAlso runningCandlePayload.SnapshotDateTime >= userSettings.TradeStartTime AndAlso
            ...
            If Me.InstrumentOption = OptionType.Straddle Then
                quantity = Me.TradableInstrument.LotSize * Me.MyParentInstrumentDetails.StraddleNumberOfLots
            ElseIf Me.InstrumentOption = OptionType.Strangle Then
                quantity = Me.TradableInstrument.LotSize * Me.MyParentInstrumentDetails.StrangleNumberOfLots
            End If
                        ...
                        ElseIf preSupertrendColor = Color.Red AndAlso supertrendColor = Color.Green AndAlso entryOrder.TimeStamp <= runningCandlePayload.PreviousPayload.PreviousPayload.SnapshotDateTime Then
                            ...
                            parameters = New PlaceOrderParameters(runningCandlePayload.PreviousPayload) With
                                {.EntryDirection = IOrder.TypeOfTransaction.Buy,
                                 ...
                                 .OrderType = IOrder.TypeOfOrder.Market}
                        Else
                            If log Then OnHeartbeat(String.Format("Entry Price:{0}. SL1:{1}, ST Color:{2}, Pre ST Color:{3}, Entry Time:{4}. LTP:{5}, Close Price:{6}. So it will not place exit order",
                                                                  entryOrder.AveragePrice, stoploss1, supertrendColor.Name, preSupertrendColor.Name, entryOrder.TimeStamp.ToString("dd-MM-yyyy HH:mm:ss"), currentTick.LastPrice, runningCandlePayload.PreviousPayload.ClosePrice.Value))
                        End If
                    End If
                End If
            Else
