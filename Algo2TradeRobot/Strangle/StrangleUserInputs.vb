...
<Serializable>
Public Class StrangleUserInputs
    Inherits StrategyUserInputs

    Public Shared Property SettingsFileName As String = Path.Combine(My.Application.Info.DirectoryPath, "StrangleSettings.Strategy.a2t")
    ...
    Public Class InstrumentDetails
        ...
        Public Property DistanceFromClose As Decimal
        Public Property StoplossMultiplier As Decimal
    End Class
            ...
            If File.Exists(filePath) Then
                ...
                If extension = ".csv" Then
                    ...
                    If instrumentDetails IsNot Nothing AndAlso instrumentDetails.Length > 0 Then
                        Dim excelColumnList As New List(Of String) From {"INSTRUMENT NAME", "NUMBER OF LOTS", "TIMEFRAME", "SUPERTREND PERIOD", "SUPERTREND MULTIPLIER", "DISTANCE FROM CLOSE", "STOPLOOS MULTIPLIER"}
                        ...
                        For rowCtr = 1 To instrumentDetails.GetLength(0) - 1
                            ...
                            Dim dstncFrmCls As Decimal = Decimal.MinValue
                            Dim slMul As Decimal = Decimal.MinValue
                            ...
                            For columnCtr = 0 To instrumentDetails.GetLength(1)
                                ...
                                ElseIf columnCtr = 1 Then
                                    If instrumentDetails(rowCtr, columnCtr) IsNot Nothing AndAlso
                                        ...
                                        If IsNumeric(instrumentDetails(rowCtr, columnCtr)) AndAlso
                                            ...
                                            nmbrOfLots = Val(instrumentDetails(rowCtr, columnCtr))
                                            If nmbrOfLots < 1 Then
                                                Throw New ApplicationException(String.Format("Number Of Lots cannot be < 1 for {0}", instrumentDetails(rowCtr, columnCtr).GetType, instrumentName))
                                            End If
                                        Else
                                ...
                                ElseIf columnCtr = 5 Then
                                    If instrumentDetails(rowCtr, columnCtr) IsNot Nothing AndAlso
                                        Not Trim(instrumentDetails(rowCtr, columnCtr).ToString) = "" Then
                                        If IsNumeric(instrumentDetails(rowCtr, columnCtr)) Then
                                            dstncFrmCls = Val(instrumentDetails(rowCtr, columnCtr))
                                        Else
                                            Throw New ApplicationException(String.Format("Distance From Close cannot be of type {0} for {1}", instrumentDetails(rowCtr, columnCtr).GetType, instrumentName))
                                        End If
                                    Else
                                        Throw New ApplicationException(String.Format("Distance From Close cannot be null for {0}", instrumentDetails(rowCtr, columnCtr).GetType, instrumentName))
                                    End If
                                ElseIf columnCtr = 6 Then
                                    If instrumentDetails(rowCtr, columnCtr) IsNot Nothing AndAlso
                                        Not Trim(instrumentDetails(rowCtr, columnCtr).ToString) = "" Then
                                        If IsNumeric(instrumentDetails(rowCtr, columnCtr)) Then
                                            slMul = Val(instrumentDetails(rowCtr, columnCtr))
                                        Else
                                            Throw New ApplicationException(String.Format("Stoploss Multiplier cannot be of type {0} for {1}", instrumentDetails(rowCtr, columnCtr).GetType, instrumentName))
                                        End If
                                    Else
                                        Throw New ApplicationException(String.Format("Stoploss Multiplier cannot be null for {0}", instrumentDetails(rowCtr, columnCtr).GetType, instrumentName))
                                    End If
                                End If
                            Next
